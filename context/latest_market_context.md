# Market Overview

Generated: 2026-07-28T20:47:53
Stage: premarket | Report date: 2026-07-28

## Market Regime

当前市场状态：`weak_breadth_or_pullback`

- Snapshot stage: premarket
- Snapshot generated at: 2026-07-28T20:47:49
- Execution boundary: L3_MANUAL_CONFIRM_REQUIRED
- Monitor fresh/stale: 47/9

## Index

- `QQQ` Nasdaq 100 ETF
  - close: 682.1199951171875
  - price: 677.32
  - premarket_price: 677.32
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: -0.70
  - premarket_volume: 340.0
  - VWAP: 689.934889369093
- `SPY` S&P 500 ETF
  - close: 739.0900268554688
  - price: 739.87
  - premarket_price: 739.87
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: 0.11
  - premarket_volume: 1386.0
  - VWAP: 740.5963557608586
- `SOXX` iShares Semiconductor ETF
  - close: 516.22998046875
  - price: 496.03
  - premarket_price: 496.03
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: -3.91
  - premarket_volume: 40.0
  - VWAP: 531.3946671353011
- `SMH` VanEck Semiconductor ETF
  - close: 548.5499877929688
  - price: 532.28
  - premarket_price: 532.28
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: -2.97
  - premarket_volume: 50.0
  - VWAP: 558.8785395996055
- `IWM` Russell 2000 ETF
  - close: 292.9100036621094
  - premarket_change_status: UNAVAILABLE

## Leaders

- `GOOGL` Alphabet
  - chain: cloud_ai_capex
  - close: 326.55999755859375
  - price: 328.00
  - premarket_price: 328.41
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: 0.57
  - premarket_volume: 120.0
  - VWAP: 326.17
  - score: 23.78
  - level: L1
  - caution: spread_missing
- `ASML` ASML Holding
  - chain: semiconductor_equipment
  - close: 1655.260009765625
  - price: 1573.65
  - premarket_price: 1573.65
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: -4.93
  - premarket_volume: 60.0
  - score: -3.22
  - level: L3_MANUAL
  - caution: below_vwap
- `PWR` Quanta Services
  - chain: data_center_power_cooling
  - close: 620.280029296875
  - premarket_change_status: UNAVAILABLE
  - score: -4.30
  - level: L3_MANUAL
  - caution: below_vwap
- `TT` Trane Technologies
  - chain: data_center_power_cooling
  - close: 483.3399963378906
  - premarket_change_status: UNAVAILABLE
  - score: -6.47
  - level: L3_MANUAL
  - caution: below_vwap
- `ANET` Arista Networks
  - chain: ai_networking_optical
  - close: 170.75999450683594
  - premarket_change_status: UNAVAILABLE
  - score: -7.00
  - level: L3_MANUAL
  - caution: below_vwap
- `GEV` GE Vernova
  - chain: data_center_power_cooling
  - close: 996.5700073242188
  - premarket_change_status: UNAVAILABLE
  - score: -7.37
  - level: L3_MANUAL
  - caution: below_vwap
- `JCI` Johnson Controls
  - chain: data_center_power_cooling
  - close: 143.38999938964844
  - premarket_change_status: UNAVAILABLE
  - score: -7.72
  - level: L3_MANUAL
  - caution: below_vwap
- `LITE` Lumentum
  - chain: ai_networking_optical
  - close: 711.9600219726562
  - premarket_change_status: UNAVAILABLE
  - score: -7.81
  - level: L3_MANUAL
  - caution: below_vwap
- `ETN` Eaton
  - chain: data_center_power_cooling
  - close: 398.6400146484375
  - premarket_change_status: UNAVAILABLE
  - score: -7.95
  - level: L3_MANUAL
  - caution: below_vwap
- `IWM` Russell 2000 ETF
  - chain: market_regime
  - close: 292.9100036621094
  - premarket_change_status: UNAVAILABLE
  - score: -8.27
  - level: L1
  - caution: below_vwap
- `KLAC` KLA
  - chain: semiconductor_equipment
  - close: 203.36000061035156
  - premarket_change_status: UNAVAILABLE
  - score: -8.45
  - level: L1
  - caution: below_vwap
- `SKHY` SK hynix ADR
  - chain: memory_hbm_storage
  - price: 136.65
  - premarket_price: 136.65
  - premarket_change_status: UNAVAILABLE
  - premarket_volume: 200.0
  - score: -8.94
  - level: L1
  - caution: below_vwap

## Technical State

- `GOOGL`
  - close: 326.55999755859375
  - price: 328.00
  - premarket_price: 328.41
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: 0.57
  - premarket_volume: 120.0
  - VWAP: 326.17
  - above_vwap: True
  - caution: spread_missing
  - execution: L1
- `ASML`
  - close: 1655.260009765625
  - price: 1573.65
  - premarket_price: 1573.65
  - premarket_change_status: UNAVAILABLE
  - premarket_change_proxy_pct: -4.93
  - premarket_volume: 60.0
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `PWR`
  - close: 620.280029296875
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `TT`
  - close: 483.3399963378906
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `ANET`
  - close: 170.75999450683594
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `GEV`
  - close: 996.5700073242188
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `JCI`
  - close: 143.38999938964844
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL
- `LITE`
  - close: 711.9600219726562
  - premarket_change_status: UNAVAILABLE
  - above_vwap: False
  - caution: below_vwap
  - execution: L3_MANUAL

## GPT Overlay Continuity

- Availability
  - latest_premarket_overlay_available: True
  - latest_postmarket_overlay_available: False
  - latest_premarket_overlay_date: 2026-07-13
  - continuity_status: new_baseline
- Latest Decision
  - decision: RISK_REDUCTION_REVIEW
  - decision_reason: 存储和半导体盘前处于宏观风险、拥挤交易、ADR与本股定价错位及杠杆去风险叠加阶段。对现有杠杆暴露先做风险检查；若无杠杆持仓则降级为NO_TRADE。

### Latest Thesis Ledger

- T-AI-CAPEX-20260713: AI资本开支基本面尚未出现明确下修，但油价、通胀和收益率上升正在提高估值风险。
- T-MEMORY-HBM-20260713: HBM供需逻辑尚未失效，但ADR溢价、拥挤持仓、杠杆产品和远期扩产使交易结构显著恶化。
- T-SEMI-EQUIPMENT-20260713: 设备链需要ASML订单和TSMC资本开支及指引确认，盘前价格变化不足以升级基本面判断。

### Latest Forecast Ledger

- F-20260713-05: 如果SOXX和SMH开盘首小时无法收复盘前跌幅的一半，且存储股继续跑输TSM和NVDA，则本轮下跌更可能属于拥挤交易去杠杆。 |
  confidence=medium |
  horizon=2026-07-13 US session | review_due=2026-07-14
  - expected: SOXX和SMH无法同步站回VWAP，存储股继续相对弱势。
  - invalidation: SOXX、SMH快速站回VWAP，存储股同步止跌且成交结构改善。
- F-20260713-06: 若油价和收益率继续上升，高估值AI与半导体将继续相对SPY承压，即使盈利预期暂未下修。 | confidence=medium_high |
  horizon=2026-07-13 to
  2026-07-14 | review_due=2026-07-14
  - expected: 半导体和高估值科技相对SPY走弱。
  - invalidation: 油价和收益率上升但半导体仍形成广泛持续的相对强势。
- F-20260713-07: SKHY ADR与韩国本股的定价差异将在未来数日继续放大波动，普通技术支撑暂时失真。 | confidence=high | horizon=1-5
  trading days |
  review_due=2026-07-20
  - expected: ADR、本股和相关杠杆产品继续出现异常价差与高波动。
  - invalidation: 价差显著收敛，套利与成交机制恢复稳定。

### Candidate Conditions

- No buy precheck candidates supplied by GPT overlay.

### Review Due Forecast Claims

- No overdue unreviewed forecast claims.

## Risk Factors

- low: data_freshness - fresh=47, stale=9, stale_ratio=16.07%
- medium: execution_boundary - Automated output is capped at L3_MANUAL_CONFIRM_REQUIRED unless
  broker realtime
  data is verified.
- medium: market_or_data_caution - below_vwap: 45
- medium: market_or_data_caution - price_stale_or_missing: 9
- medium: market_or_data_caution - spread_too_wide_or_missing: 2
- info: market_breadth - latest breadth cache present; generated_at=2026-07-09T23:31:33

## Watch Items

- `NVDA` NVIDIA
  - tier: A_CORE
  - action: research_watch
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `TT` Trane Technologies
  - tier: B_HIGH_BETA
  - action: core_watch_wait_for_pullback
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `SKHY` SK hynix ADR
  - tier: B_HIGH_BETA
  - action: research_watch
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `TSM` TSMC ADR
  - tier: A_CORE
  - action: long_logic_only_wait_for_trend_repair
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `JCI` Johnson Controls
  - tier: B_HIGH_BETA
  - action: research_watch
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `ANET` Arista Networks
  - tier: B_HIGH_BETA
  - action: watch_for_reclaim_and_support
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `AAPL` Apple
  - tier: C_RESEARCH
  - action: core_watch_wait_for_pullback
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `HPE` Hewlett Packard Enterprise
  - tier: C_RESEARCH
  - action: core_watch_wait_for_pullback
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `DELL` Dell Technologies
  - tier: C_RESEARCH
  - action: core_watch_wait_for_pullback
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `AVGO` Broadcom
  - tier: A_CORE
  - action: long_logic_only_wait_for_trend_repair
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `ETN` Eaton
  - tier: C_RESEARCH
  - action: research_watch
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.
- `IWM` Russell 2000 ETF
  - tier: INDEX_REGIME
  - action: use_as_regime_check
  - downgrade_if: Downgrade if earnings/order thesis is disproved, valuation rises without
    growth support,
    news risk becomes hard negative, or price loses medium-term trend support.

## Questions For Investment Committee

- Is current strength broad-based, or driven by a narrow group of AI, semiconductor, and
  infrastructure
  leaders?
- Which moves are supported by durable fundamental catalysts, and which are mainly news spikes
  or crowded
  trades?
- Are high-score names extended away from VWAP/opening range, or still near a reasonable
  support-check zone?
- Please judge whether the support behind these leaders is durable: GOOGL, ASML, PWR, TT, ANET.
- Separate data-quality/execution-boundary risks from true market risks.

## Source Files

- Redacted in public bridge.
